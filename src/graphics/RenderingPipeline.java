...
public class RenderingPipeline //extends Thread
{
	...
	private final Vect3D planeW00;
	private final Vect3D plane0H0; 
	
	private final static Vect3D PLANE_000 = new Vect3D(0f, 0f, 0f);
	private final static Vect3D PLANE_010 = new Vect3D(0f, 1f, 0f);
	private final static Vect3D PLANE_0m10 = new Vect3D(0f, -1f, 0f);
	private final static Vect3D PLANE_100 = new Vect3D(1f, 0f, 0f);
	private final static Vect3D PLANE_m100 = new Vect3D(-1f, 0f, 0f);
	private final static Vect3D PLANE_001 = new Vect3D(0f, 0f, 1f);
	
	private final static Vect3D PLANE_P = new Vect3D(0f, 0f, 0.1f); // was set to 0.001
							 ...
							 boolean showWireframe)
	{
		...
		planeW00 = new Vect3D(width - 1f, 0f, 0f);
		plane0H0 = new Vect3D(0f, height - 1f, 0f);
	}
	...
	public void run()
	{
		...
		for(Triangle tri : mesh.tris)
		{
			...
			if(MathUtils.dotProd(normal, vCameraRay) < 0f)
			{
				...
				Triangle[] clipped = MathUtils.clipAgainstPlane(PLANE_P, PLANE_001, triViewed);
		...
		for(int i = 0; i < visibleTris.size(); i++)
		{
			...
			for(int p = 0; p < 4; p++)
			{
				...
				while(nNewTris > 0)
				{
					...
					switch(p)
					{
						case 0: clipped = MathUtils.clipAgainstPlane(PLANE_000, PLANE_010, test); break;
						case 1: clipped = MathUtils.clipAgainstPlane(plane0H0,  PLANE_0m10, test); break;
						case 2: clipped = MathUtils.clipAgainstPlane(PLANE_000, PLANE_100, test); break;
						case 3: clipped = MathUtils.clipAgainstPlane(planeW00,  PLANE_m100, test);
					}
